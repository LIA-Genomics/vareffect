...
//! `crates/vareffect/scripts/generate_vep_ground_truth.py`), runs vareffect on
...
fn vep_large_concordance() {
    ...
    if !tsv_path.exists() {
        ...
        eprintln!(
            "      Run: uv run crates/vareffect/scripts/generate_vep_ground_truth.py \\
             --input data/clinvar/clinvar.vcf.gz \\
             --output {} \\
             --max-variants 50000",
            tsv_path.display()
        );
